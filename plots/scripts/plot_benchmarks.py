...
def save_plot(df_rbr,df_br,outdir,plot,sample,rule):
    ...
    plt.plot(df_br["ref"] * 2, df_br[plot], marker = 's', color = "royalblue", label='BigRePair', linewidth=2.0, markersize=8.0)
    plt.plot(df_rbr["ref"] * 2, df_rbr[plot], marker = 'D', color = "darkorange", label='Re$^2$Pair', linewidth=2.0, markersize=8.0)
    plt.xticks(df_rbr["ref"] * 2)
    ...
    if plot == "sec":
        ...
        plt.xlabel('Haplotypes', fontsize='x-large')
    if plot == "cpu_time":
